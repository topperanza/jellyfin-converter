---
name: commit-push
description: Commit and push verified milestone work with milestone-aware tracking and no history rewriting
---

> Canonical source: docs/codex/usage-prompts/07A-commit-push.md
...
- `solo-direct-push`: single maintainer, direct push to main after local verification; no PR required.
  - If current branch is not `main`: push current branch, then proceed to `07B-squash-merge.md`.
  - If current branch is already `main`: push `main`; this prompt ends the workflow.
- `protected/team`: branch protection active, PRs required to merge into main.
  - Push the current branch to origin.
  - Open a PR against `main` if none exists (do not push directly to `main`).
  - This prompt ends after the push; merge is completed via PR review.
...
2. Read workflow mode from `docs/codex/REPO_OVERVIEW.md`.
3. Confirm whether the current branch is `main` or a non-main branch.
4. Verify required tracking/documentation surfaces using `DOC_SYNC_MATRIX.md`.
5. Run the lightest sufficient validation:
   - narrow checks first
   - `bash scripts/check-fast.sh`
   - `bash scripts/check-changed.sh` if present and relevant
   - `bash scripts/check-full.sh` only if justified
6. Review the final diff and ensure the scope matches the verified work.
7. Prepare commit grouping.
8. Write proper commit message(s):
   `<area/scope>: <imperative summary>`
9. Stage only intended files and commit.
10. Push based on workflow mode:
    - `solo-direct-push` + feature branch: push current branch
    - `solo-direct-push` + on `main`: push `main`
    - `protected/team`: push current branch, open PR if none exists
...
- If workflow mode is `protected/team`:
  `No next prompt needed.` (merge via PR)
