...
# Mock helpers to avoid sourcing full environment dependencies if possible
# But media_filters.sh needs compat.sh. 
# We'll source the actual file to test the real logic.

source "$SCRIPT_DIR/lib/media_filters.sh"

# Mock to_lower/to_upper if compat.sh wasn't loaded correctly (but it should be)
if ! command -v to_lower >/dev/null; then
  to_lower() { echo "$1" | tr '[:upper:]' '[:lower:]'; }
  to_upper() { echo "$1" | tr '[:lower:]' '[:upper:]'; }
  have_bash_ge_4() { return 1; } # Force Bash 3 path if applicable
fi
...
info_A=$'2,subrip,eng,English,0,0\n3,subrip,rus,Russian,0,0'
test_selection "Preferred (Eng) over Russian" "$info_A" "-map 0:2" "-map 0:3" || exit 1

# Case B: Only Russian (Fallback)
info_B=$'3,subrip,rus,Russian,0,0'
test_selection "Only Russian (Fallback)" "$info_B" "-map 0:3" "" || exit 1

# Case C: Non-Russian (French) vs Russian -> Prefer French
info_C=$'4,subrip,fra,French,0,0\n5,subrip,rus,Russian,0,0'
test_selection "Non-Russian (French) over Russian" "$info_C" "-map 0:4" "-map 0:5" || exit 1

# Case D: Forced subs
info_D=$'6,subrip,spa,Spanish,0,1\n7,subrip,rus,Russian,0,0'
test_selection "Forced subs kept" "$info_D" "-map 0:6" "" || exit 1

# Case E: Commentary
info_E=$'8,subrip,und,Director Commentary,0,0'
test_selection "Commentary kept" "$info_E" "-map 0:8" "" || exit 1


# 2. Integration Test: No Subtitles Crash
echo "--- Integration Test: No Subtitles (Crash Check) ---"

INPUT_VIDEO="$TEST_TEMP_DIR/input_nosubs.mkv"
...
OUTPUT_VIDEO="$TEST_TEMP_DIR/out/input_nosubs.mkv"
